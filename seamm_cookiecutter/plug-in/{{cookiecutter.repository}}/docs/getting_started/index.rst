...
Installation
============
The {{ cookiecutter.step }} step is probably already installed in your SEAMM
...
check for `{{ cookiecutter.repository.replace("_", "-") }}`. If it is not installed, or
...
The non-graphical installer is also straightforward::
  ...
  seamm-installer install --update {{ cookiecutter.repository.replace("_", "-") }}
